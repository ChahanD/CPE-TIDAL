...
<body>
    ...
    try {
        ...
        } elseif (isset($_GET['meridien']) && !empty($_GET['meridien'])) {
            $stmt_patho_desc = $conn->prepare("SELECT * FROM patho WHERE mer = :meridien");
            $stmt_patho_desc->execute(['meridien' => $_GET['meridien']]);
        } else {
